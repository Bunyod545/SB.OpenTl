...
namespace OpenTl.ClientApi.Services
{
    ...
    internal class UpdatesService : IUpdatesService, IAutoUpdatesHandler, IDisposable
    {
        ...
        public void StartReceiveUpdates(TimeSpan updatePeriod)
        {
            ...
            var updateState = ClientSettings.ClientSession.UpdateState;
            if (updateState != null)
            {
                var diff = GetUpdatesFromState(updateState).Result;
                HandleManualsUpdates(diff).Wait();
            }
            else
            {
                ClientSettings.ClientSession.UpdateState = (TState)GetCurrentStateAsync().Result;
            }
